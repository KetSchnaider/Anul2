...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($requestData["year_id"]) || empty($requestData["zone_id"]) || empty($requestData["category_name"]) || !isset($requestData["data"])) {
    ...
    if ($yearExistsResult->num_rows === 1 && $zoneExistsResult->num_rows === 1) {
        $insertQuery = "INSERT INTO waste_statistics (year_id, zone_id, category_name, data) VALUES (?, ?, ?, ?)";
        $insertStmt = $conn->prepare($insertQuery);

        $insertStmt->bind_param("isss", $requestData["year_id"], $requestData["zone_id"], $requestData["category_name"], $requestData["data"]);
        $insertStmt->execute();

        if ($insertStmt->affected_rows > 0) {
            http_response_code(201); // Created
            echo json_encode(["message" => "Waste statistics added successfully"]);
        } else {
            http_response_code(500); // Internal Server Error
