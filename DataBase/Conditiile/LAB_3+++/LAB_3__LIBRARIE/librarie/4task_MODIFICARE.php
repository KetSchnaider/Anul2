...
<body>
	<div>			
<H1 style ="text-align:center"> MODIFICARE  DATE IN TABELUL AUTOR </H1>
<?PHP 
require_once 'connectare.php';
require '1task_AFISARE_TABEL_AUTOR.php';  
	$nume_autor = $eroarenumeautor= $nume_autor_m=$submit="";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") 
{
  if (empty($_POST["nume_autor"])) 
  {
    ...
    if (!preg_match("/^[a-zA-Z ]*$/",$nume_autor)) 	{ $eroarenumeautor = "Se introduc doar litere si spatii"; }
  }
} 
if ($_SERVER["REQUEST_METHOD"] == "POST") 
{
  if (empty($_POST["nume_autor_m"]))   {    $eroarenumeautor = "Se cere a fi introdus nume_autor_m";   } 
  else 
  {
    $nume_autor_m = procesare_input($_POST["nume_autor_m"]);
    // Verificare daca numele contine litere si spatii
    if (!preg_match("/^[a-zA-Z ]*$/",$nume_autor_m)) 	{ $eroarenumeautor = "Se introduc doar litere si spatii";
    }
  }
}
...
?>
		...
		<span class="atentionare">* <?php echo $eroarenumeautor;?></span>
		Nume modif  :   	<input type="text" name="nume_autor_m" size="15">  <br/>
		<input type="submit" name="submit" value="Submit">  
	</form> 
<?php
if ((($nume_autor<>"")and ($nume_autor_m<>"")))
{
/* LUCRUL CU COMANDA SELECT */
$sql = "UPDATE autor SET nume_autor='$nume_autor_m' WHERE nume_autor ='$nume_autor'";
$result= mysqli_query($link, $sql);
if ($result)
{
	ECHO "<p align='center'> <font color=BLUE >DATELE AU FOST MODIFICATEE</p>";
require '1task_AFISARE_TABEL_AUTOR.php';  
}
else
{
...
ELSE
{
ECHO "<p align=center> <font color=red > INTRODUCETI DATELE PENTRU MODIFICARE </font></p>";
}	 
?>  
